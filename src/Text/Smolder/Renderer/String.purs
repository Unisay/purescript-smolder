module Text.Smolder.Renderer.String (renderWithEvents, render) where
...
fromStream = go ""
  ...
  go result cof = case (head cof), (tail cof) of
    _, Nothing -> result
...
escape m = fromStream <<< extend escapeS <<< toStream
  ...
  startsEntity (Just w) = case head w, tail w of
  ...
  checkTail allowed = flip evalState false <<< checkTail'
    ...
    checkTail' w = case toCharCode $ head w of
      cc
        | cc `elem` allowed -> do
          put true
...
renderItem :: ∀ e. (EventHandler e -> String) -> MarkupM e ~> State String
renderItem er (Element ns name children attrs eventHandlers rest) =
  let
    c = renderWithEvents er children

    b =
      "<" <> name <> showAttrs name attrs
        <> guard (CL.length eventHandlers > 0)
            (surround " " $ renderEventHandler <$> eventHandlers)
        <> ( if length c > 0 || (ns == HTMLns && not Set.member name voidElements) then
              ">" <> c <> "</" <> name <> ">"
            else
              "/>"
          )
  ...
  where
  renderEventHandler h@(EventHandler eventName event) =
    eventName
      <> "="
      <> "\""
      <> escape escapeMap (er h)
      <> "\""

renderItem _ (Content text rest) =
  state \s ->
    Tuple rest $ append s $ escape escapeMap text

renderItem _ (Doctype text rest) =
  state \s ->
    Tuple rest $ append s $ "<!DOCTYPE " <> text <> ">"

renderItem _ (Empty rest) = pure rest

-- | Render markup as an HTML string with event renderer
renderWithEvents :: ∀ e. (EventHandler e -> String) -> Markup e -> String
renderWithEvents er m = execState (foldFree (renderItem er) m) ""

-- | Render markup as an HTML string.
render :: ∀ e. Markup e -> String
render = renderWithEvents (const mempty)
